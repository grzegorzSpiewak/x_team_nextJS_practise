...
const renderNews = (items) => {
  return items.map((item) =>
    <li className="unleash-feed__item unleash-feed__item--devs" itemprop="award">
...
const renderDevs = (devs) => {
  return devs.map((dev) =>
    <article className="cavalry__item cavalry__item--devs">
      <ul className="cavalry__item__list cavalry__item__list--devs" itemscope itemtype="http://schema.org/Person">
        <li className="cavalry__item__pic cavalry__item__pic--devs">
          <a className="cavalry__item__pic__link" href={`${dev.link || '#'}`} target="_blank">
            <img src={`${config.staticPath}/images/devs/${dev.pic}`} alt={`${dev.name}`} className="cavalry__item__pic__img cavalry__item__pic__img--devs" itemprop="image" />
          </a>
        </li>
  ...
  <section className="block cavalry cavalry--devs">
    <div className="wrap">
      ...
      <div className="cavalry__cta cavalry__cta--devs">
         { Button(props.button) }  //Add button props to data
      </div>
    </div>
  </section>
)
